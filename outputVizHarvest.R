...
print(m + theme_dark() +
          ...
          labs(title = paste0("Analyse des récoltes réalisées - ", scenario),
...
print(m + theme_dark() +
          ...
          labs(title = "Analyse de la proportion des récoltes effectuée aprÃ¨s feu (récupération)",
               #subtitle = paste0(percentile, "e percentile"),
               subtitle = paste0("Chaque courbe représente la fraction du total des superficies récoltées correspondant Ã  la coupe de récupération\n",
                                 "pour un percentile donné au sein d'un ensemble de ",nSims, " simulations."),
               # caption = paste0("Àge min. de récolte (sauf récup.) - Épinette noire: 90 ans\n",
               #                  "Pin gris: 76 ans\n",
               #                  "Vol. marchand min.: 50 m3/ha (Récup.: 70 m3/ha)\n",
...
m <- ggplot(df, aes(x = year + 2015, y = value*100,
                    linetype = variable)) +
    geom_line(size = 0.5) +
    scale_linetype_manual("Ã‰cart toléré\n",
...
print(m + theme_dark() +
          ...
          labs(title = "Analyse de risque de rupture d'approvisionnement",
               ...
               subtitle = paste0("Probabilité de ne pas atteindre la cible de récolte au moins une fois en fonction de différents niveaux de\n",
                                 "tolérance par rapport Ã  cette cible - estimé sur la base d'un ensemble de ", nSims, " simulations."),
               caption = paste0("Àge min. de récolte (sauf récup.) - Ã‰pinette noire: 90 ans\n",
                                "Pin gris: 76 ans\n",
                                "Vol. marchand min.: 50 m3/ha (Récup.: 70 m3/ha)\n",
